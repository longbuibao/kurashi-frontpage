  ...
  return (
    <div className='flex flex-row gap-3 bg-main-phu-kien p-3 rounded-xl w-1/3 max-md:w-full'>
      <div className='flex flex-col gap-5 justify-center w-1/2 ml-5'>
        <div className='text-xl max-md:text-[1rem]'>{subCategory.content}</div>
        <Link className='p-2 rounded-lg flex flex-row items-center gap-5 mt-5' href={subCategory.url as any as UrlObject}>
          <Image src='/images/RightArrow.svg' className='max-md:hidden' width={20} height={20} alt='Khám phá phụ kiện bếp nam châm' />
          <Image className='hidden max-md:block' src='/images/RightArrow.svg' width={10} height={10} alt='Khám phá phụ kiện bếp nam châm' />
          <div className='max-md:text-[1rem]'>Khám phá</div>
        </Link>
      </div>
